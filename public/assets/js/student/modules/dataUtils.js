...
export async function checkAssessmentRespond(pid) {
    const data = await getJWTtoken();
    const token = data.token
    return new Promise((resolve, reject) => {
        $.ajax({
            url: `/api/v1/posts/assessment/status?pid=${pid}`,
            method: 'GET',
            beforeSend: function(xhr) {
                xhr.setRequestHeader('Authorization', `Bearer ${token}`)
            },
...
export async function checkSubmissionRespond(pid) {
    const data = await getJWTtoken();
    const token = data.token
    return new Promise((resolve, reject) => {
        $.ajax({
            url: `/api/v1/posts/submission/status?pid=${pid}`,
            method: 'GET',
            beforeSend: function(xhr) {
                xhr.setRequestHeader('Authorization', `Bearer ${token}`)
            },
