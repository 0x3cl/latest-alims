        ...
        <div id="dashboard" class="mx-2">
            <div class="container">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="/api/single/add/course" method="post">
                            <div class="row mt-4">
                                <?php 
                                     if (!empty($response)) {
                                ...
                                <div class="col-12 col-md-12 mb-3">
                                    ...
                                    <input type="text" name="code" id="code" class="form-control <?php echo (!empty($response["message"]["code"]) ? 'is-invalid' : '') ?>" placeholder="eg. BSIT" value="<?php echo (!empty($response) && isset($response["fields"]["code"]) ? $response["fields"]["code"] : ''); ?>">
                                    <?php echo (!empty($response["message"]["code"]) ? '<small class="invalid-feedback">'.ucfirst($response["message"]["code"]).'</small>' : '') ?>
                                </div>
                                <div class="col-12 col-md-12 mb-3">
                                    <label>Name <span class="text-danger">*</span> </label>
                                    <input type="text" name="name" id="name" class="form-control <?php echo (!empty($response["message"]["name"]) ? 'is-invalid' : '') ?>" placeholder="eg. Bachelor of Science in Information Technology" value="<?php echo (!empty($response) && isset($response["fields"]["name"]) ? $response["fields"]["name"] : ''); ?>">
